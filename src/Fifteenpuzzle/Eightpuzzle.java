...
public class Eightpuzzle
{
        ...
        for( Integer step : step_values ) {
            ...
            for( int i = 1; i <= 4; i++ ) {
                ...
                switch( i ) {
                    case 1:
                        // pos 1, 2, 3 => ~UP
                        positions.add( 1 ); positions.add( 2 ); positions.add( 3 );
                        break;
                    case 2:
                        // pos 1, 4, 7 => ~LEFT
                        positions.add( 1 ); positions.add( 4 ); positions.add( 7 );
                        break;
                    case 3:
                        // pos 3, 6, 9 => ~RIGHT
                        positions.add( 3 ); positions.add( 6 ); positions.add( 9 );
                        break;
                    case 4:
                        // pos 7, 8, 9 => ~DOWN
                        positions.add( 7 ); positions.add( 8 ); positions.add( 9 );
                        break;
                    default:
                ...
                for( Integer position : position_values ) {
                    ...
                    for( Integer tile : tile_values ) {
                        ...
                        if( tile != tile_values.length && step < step_values.length ) {
                            ...
                            if( operation == Operation.UP && position < 4 ) {
                                continue;
                            } else if( operation == Operation.DOWN && position > 6 ) {
                                continue;
                            ...
                            switch( operation ) {
                                case UP:
                                    constants3.add( new Constant<Integer>( "position", position - 3 ) );
                                    pos_2 = position - 3;
                                    break;
                                case DOWN:
                                    constants3.add( new Constant<Integer>( "position", position + 3 ) );
                        ...
                        if( tile != tile_values.length && step < step_values.length ) {
                            ...
                            //for(int i = 0; i < 2; i++ ) {
                                ...
                                for(Integer remaining_position : remaining_positions ) {
                                    ...
                                    constants3.add( new Constant<Integer>( "step", step ) );
                                    conditions.add( new Relation( "TP", /*i == 0*/ true, constants3 ) );
                                    ...
                                    constants4.add( new Constant<Integer>( "step", step + 1 ) );
                                    outcomes.add( new Relation( "TP", /*i == 0*/ true, constants4 ) );
    ...
    public static void main( String[] args )
    {
        ...
        new VSIDS().analyzeFile( args[0]  + ".enc" );
